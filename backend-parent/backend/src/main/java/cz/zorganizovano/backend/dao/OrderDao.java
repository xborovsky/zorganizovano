...
public interface OrderDao extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT o FROM Order o JOIN FETCH o.customer "
            + "WHERE (o.invoiceSent IS NULL OR (?1 = true AND o.invoiceSent IS NOT NULL)) "
            + "AND (o.storno IS NULL OR (?2 = true AND o.storno IS NOT NULL)) "
            + "ORDER BY o.created DESC")
    List<Order> findOrders(boolean invoiceSent, boolean storno);
    
    @Query("SELECT o FROM Order o JOIN FETCH o.customer WHERE o.id IN(?1) ORDER BY o.created DESC")
    List<Order> findOrdersForReport(List<Long> orderIds);

    @Query("SELECT o FROM Order o JOIN FETCH o.customer WHERE o.paymentReceived IS NULL AND o.storno IS NULL")
